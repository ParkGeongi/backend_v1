...
class ExtractColor(object):
    ...
    def hook(self, img):
        ...
        common_color_rgb, palette, max_rgb, min_rgb = self.palette_perc(img)
        color_name = self.closest_color_name(common_color_rgb)
        return color_name
    ...
    def palette_perc(self, target_item):
        ...
        max_rgb = np.amax(k_cluster.cluster_centers_, axis=0)
        min_rgb = np.amin(k_cluster.cluster_centers_, axis=0)

        return common_color_rgb, palette, max_rgb, min_rgb
